...
document.addEventListener("DOMContentLoaded", () => {
  ...
  WebT.settings = {
    anchor_scroll_offset: -74,
    header_sticky_pos: 25,
    dummy: null
  };
  ...
  (() => {
    WebT.elements.scroll_links.forEach(anchor => {
      anchor.addEventListener('click', function (e) {
        ...
        const offset = WebT.settings.anchor_scroll_offset,
  ...
  /* Comments counter */
  (() => {
    let comments_amount_text = document.querySelectorAll('.js-comments-count');
    let comments_amount = document.querySelectorAll('.comments-wrapper__item').length;

    for (let i = 0; i < comments_amount_text.length; i++) {
      comments_amount_text[i].innerHTML = comments_amount;
    }
  })();
  ...
  (() => {
    ...
    for (let i=0; i < comments_close_button.length; i++) {
      comments_close_button[i].addEventListener('click', () => {
        document.body.classList.remove('--popup-opened');
      });
    }
  })();
